...
function NewPurchaseBillContent() {
  ...
  const poId = searchParams.get('poId'); // Optional: link to purchase order
  ...
  // Load purchase order if poId provided
  useEffect(() => {
    if (purchaseOrderId) {
      const fetchPO = async () => {
        try {
          const res = await fetch(`/api/v1/purchases/orders/${purchaseOrderId}`);
            ...
            if (po.lines && po.lines.length > 0) {
              setLines(
                po.lines.map((line: any) => {
                  ...
                  const vatAmount = subtotal * (Number(line.taxRate) / 100);
                  const total = subtotal + vatAmount;
                  return {
                    productId: line.productId,
                    description: line.product?.nameAr || line.description || '',
  ...
  useEffect(() => {
    if (supplierId) {
      const fetchPOs = async () => {
        try {
          const res = await fetch(`/api/v1/purchases/orders?supplierId=${supplierId}&status=APPROVED`);
          const data = await res.json();
          if (data.success) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch('/api/v1/purchases/bills', {
        method: 'POST',
      ...
      if (data.success) {
        router.push(`/purchases/bills/${data.data.id}`);
      } else {
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">إنشاء فاتورة شراء جديدة من مورد</p>
          </div>
        </div>
              ...
              <div className="border rounded-lg overflow-x-auto">
                <Table>
                  ...
                  {lines.length > 0 && (
                    <TableFooter>
                      <TableRow>
                        <TableCell colSpan={5} className="text-end font-medium">
                          المجموع الفرعي:
                        </TableCell>
                        <TableCell className="text-end font-medium">
                      ...
                      <TableRow>
                        <TableCell colSpan={5} className="text-end font-bold text-lg">
                          الإجمالي:
                        </TableCell>
